...
function createTimeoutAbortSignal(timeoutMs, task) {
...
function normalizeTimeoutError(err, signal, timeoutMs) {
...
async function streamText({ model, system, messages, maxTokens, temperature, onToken, timeoutMs }) {
  const { registry, streamText: aiStreamText } = await loadRegistry();
  const timeout = createTimeoutAbortSignal(timeoutMs, 'stream');
  ...
  try {
    ...
    throw normalizeTimeoutError(err, timeout.signal, timeoutMs);
  } finally {
    timeout.clear();
  }
}

async function completeText({ model, system, messages, maxTokens, temperature, timeoutMs }) {
  const { registry, generateText } = await loadRegistry();
  const timeout = createTimeoutAbortSignal(timeoutMs, 'completion');
...
async function completeJson({ model, system, messages, schema, maxTokens, temperature, timeoutMs }) {
  const { registry, generateObject, jsonSchema } = await loadRegistry();
  const timeout = createTimeoutAbortSignal(timeoutMs, 'json');
