...
- **Non-combat turns:** 3 calls:
  1. **Sonnet (streamed)** — narration + 3 options (~800 token prompt, creative only, no structured blocks)
...
- **Combat turns:** Zero API calls — server assembles narration from monster templates + combat engine results
  ...
  - Templates lazily generated by Haiku on first use, cached in `monster_templates` DB table
  - Generic fallback templates by creature type ship with codebase (`templates/generic-templates.json`)
- **Legacy path:** `legacyCallClaude()` preserved behind feature flag for rollback. Falls back automatically on pipeline error.
- **Corrections:** Validation violations queued in `gs.pendingCorrections[]`, injected as `[CORRECTION: ...]` in next turn's user message
- **See also:** `docs/prompt-engineering-log.md` — decision log of all prompt instructions tried and what worked/failed

### Legacy System Prompt Strategy (when SPLIT_PIPELINE=false)
