#!/usr/bin/env node
/**
 * Full Narrative Capture - Actual AI-Generated DM Text
 * Captures real Claude API responses + player actions
 */
...
const GAME_ID = `narrative-${Date.now()}`;
...
async function captureNarrative() {
  return new Promise((resolve) => {
    ...
    socket.on('connect', async () => {
      ...
      // Listen for DM narration (streaming)
      socket.on('dm_stream_chunk', (data) => {
        if (data.text) {
          dmNarrationBuffer += data.text;
          process.stdout.write('.');
        }
      });
      
      // Listen for DM message end
      socket.on('dm_stream_end', (data) => {
        if (dmNarrationBuffer) {
          if (turnData.length > 0) {
            const lastTurn = turnData[turnData.length - 1];
            lastTurn.dmNarration = dmNarrationBuffer;
            recordLog(`\n\n🎭 DM: ${dmNarrationBuffer}\n`);
          }
        }
        dmNarrationBuffer = '';
      });
      
      // Fallback: listen to dm_message directly
...
async function main() {
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║  FULL NARRATIVE CAPTURE - LIVE AI NARRATION      ║');
  console.log('║  Capturing real Claude DM responses              ║');
  console.log('╚══════════════════════════════════════════════════╝\n');
...
`;
  ...
  fs.writeFileSync('/Users/aron/Dropbox (Personal)/claude/dnd-server/FULL-NARRATIVE-LOG.txt', fullDoc);
  console.log('📋 Full narrative log saved: FULL-NARRATIVE-LOG.txt');
  console.log(`📊 Document contains: ${log.length} entries`);
  console.log(`📖 Turns captured: ${turnData.length}\n`);
}
